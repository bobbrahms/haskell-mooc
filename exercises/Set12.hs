...
fmap2 abfunc a = fmap (\x -> fmap abfunc x) a

fmap3 :: (Functor f, Functor g, Functor h) => (a -> b) -> f (g (h a)) -> f (g (h b))
fmap3 abfunc a = fmap (\x -> fmap2 abfunc x) a
...
instance Functor TwoList where
  ...
  fmap f (TwoNode a b (rest)) = TwoNode (f a) (f b) (fmap f rest)
...
count n xs =      -- sum (map ff (toList xs))
  foldr ff 0 xs
  where
    ff a y = if a == n then y+1 else y
...
inBoth fa ga =
  filter (\x -> elem x (toList ga)) (toList fa)
...
instance Foldable List where
  ...
  foldr f accum (LNode a as) = f a (foldr f accum as)
...
instance Foldable TwoList where
  foldr _ accum TwoEmpty = accum
  foldr f accum (TwoNode a b rest) = f a (f b (foldr f accum rest))
...
instance Functor Fun where
  fmap f (Fun fi) = Fun (fmap f fi)
-- alternatively
-- fmap f (Fun fi) = Fun (f . fi)
...
sumTree :: Monoid m => Tree m -> m
sumTree (Leaf)              = mempty
sumTree (Node a left right) = sumTree left <> a <> sumTree right
