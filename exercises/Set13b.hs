...
visit maze loc =
  do state <- get
     when (notElem loc state) $ do
       modify (\xs -> loc:xs)
       mapM_ (visit maze) x
         where (Just x) = lookup loc maze
...
path maze place1 place2 = do
  let start = if place1 == "" then "Entry" else place1
      endpoints = snd $ runState (visit maze start) []
  place2 `elem` endpoints
