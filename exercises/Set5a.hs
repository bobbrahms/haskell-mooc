...
totalPrice (MkShoppingEntry _ singlePrice count) =
  singlePrice * (fromIntegral count)
...
buyOneMore (MkShoppingEntry n s c) = (MkShoppingEntry n s (c + 1))
...
setName name p = MkPerson (age p) name

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age p = MkPerson age (name p)
...
data UpDown = DownTicker Int | UpTicker Int
...
tick :: UpDown -> UpDown
tick (UpTicker n) = UpTicker (n + 1)
tick (DownTicker n) = DownTicker (n - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (UpTicker n) = DownTicker n
toggle (DownTicker n) = UpTicker n
...
rgb (Mix a b) = map (\(x, y) -> (x + y)/2) (zip (rgb a) (rgb b))
rgb (Invert a) = map (\x -> (1.0 - x)) (rgb a)
...
toNat n
 | n < 0 = Nothing
 | n == 0 = Just Zero
 | otherwise = Just (recurN n)
   where
     recurN 0 = Zero
     recurN n = PlusOne (recurN (n-1))

-- toNat :: Int -> Maybe Nat
-- toNat 0 = Just Zero
-- toNat n
--  | n > 0 = Just (PlusOne (toNat (n-1)))
--  | otherwise = Nothing
...
toBin i
  | i == 0 = O End
  | i == 1 = I End 
  | mod i 2 == 0 = O (toBin (div i 2))
  | otherwise = I (toBin (div i 2))
