...
allValues condition (Node a l r)
...
set (StepL:ss) val (Node a l r) = Node a (set ss val l) r
set (StepR:ss) val (Node a l r) = Node a l (set ss val r) 
...
search val (Node a l r)
  ...
  | otherwise = thread (search val l) (search val r)
    where
      ...
      thread (Just ls) Nothing = Just(StepL:ls)
      thread Nothing (Just rs) = Just(StepR:rs)
