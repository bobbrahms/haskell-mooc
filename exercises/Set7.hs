...
member a (Set (x:xs)) =
  if a == x
  then True
  else member a (Set xs)
          
-- add a member to a set
add :: Ord a => a -> Set a -> Set a
add y (Set xs) = Set (addInto y xs)
  where
    addInto :: Ord a => a -> [a] -> [a]
    addInto y [] = [y]
    addInto y (x:xs) =
...
data State = Start | Egged | Floured | Sugared | FSed | Mixed | Finished | Error
  deriving (Eq,Show)
...
step Egged _ = Error
step Floured AddSugar = FSed
step Floured _ = Error
step Sugared AddFlour = FSed
...
average (x :| []) = x
average (x :| xs) = sum (x:xs) / (fromIntegral (length xs + 1))

------------------------------------------------------------------------------
-- Ex 5: reverse a NonEmpty list.

reverseNonEmpty :: NonEmpty a -> NonEmpty a
reverseNonEmpty (x :| xs) = head r :| tail r
  where r = reverse (x:xs)
...
passwordAllowed str (MinimumLength n) = length str >= n
passwordAllowed str (ContainsSome chars) = any (\x -> x `elem` str) chars
passwordAllowed str (DoesNotContain chars) = not (passwordAllowed str (ContainsSome chars))
...
data Arithmetic = Literal Integer | Addition Arithmetic Arithmetic | Multiplication Arithmetic Arithmetic
  deriving Show
